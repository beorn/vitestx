...
A streaming Vitest reporter that renders test results as colored dots using [hightea](https://github.com/beorn/hightea) (React terminal UI).
...
- **Incremental rendering** via `useSyncExternalStore` + hightea `flush()` -- dots appear as tests complete
...
- **Console capture** -- stdout/stderr captured via hightea `<Console />`
